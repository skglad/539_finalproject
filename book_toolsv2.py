...
def new_book_input(row):
    if Book.query.filter_by(title= row['original_title']).first():
        ...
        book = Book(title = row['original_title'], author_id=author.id, rating = row['average_rating'], image_link = row['image_url'], year_published = row['original_publication_year'], isbn = row['isbn'] )
        session.add(book)
        session.commit()
...
@app.route('/')
def welcome():
    return render_template('backgroundv2.html')
...
def index():
    ...
    return render_template('informationv2.html')
...
@app.route('/author')
def author_form():
    return render_template('author-formv2.html')
...
def search_form_post():
    ...
    return render_template ('search-returnv2.html', title_list = title_list)


#route with form that returns image of the book
@app.route('/bookcover')
def cover_form():
    return render_template('cover-formv2.html')
...
def cover_form_post():
    ...
    return render_template ('imagev2.html', book_image = book_image)
